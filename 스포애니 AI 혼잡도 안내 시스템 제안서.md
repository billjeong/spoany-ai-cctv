...
본 제안서는 스마트폰 rPPG 센싱의 기기별 성능 파편화 한계를 정면으로 돌파하기 위해, 환경 통제가 가능한 스포애니 오프라인 공간 내부의 기존 CCTV 인프라를 레버리지합니다. 고성능 엣지 AI 연산 장치(Jetson Orin Super)와 안드로이드 기반의 화면 제어 허브를 결합하여, 회원권 재등록률을 획기적으로 견인할 '공간별 맞춤형 AI 혼잡도 안내 시스템'의 상세 아키텍처 및 공간별 알고리즘 작동 로직을 제안합니다\[cite: 2, 3\].
...
* **AI 탐지 로직:**  
  * 젯슨 오린(Jetson Orin)이 기존 CCTV 카메라 피드를 분석할 때, 전체 화면이 아닌 **각 기구의 안장 및 구동 구역을 개별 ROI(Region of Interest, 관심영역)로 정밀 분할 매핑**합니다.  
  * YOLO 계열의 경량 객체 검출 모델을 가동하여 ROI 영역 내에 '움직이는 인체 객체(Human Object)'가 ![][image1] 이상 지속적으로 감지될 시 해당 기구를 '사용 중' 상태로 판정합니다.  
  * 단순히 사람이 기구 앞을 지나가는 노이즈 패턴은 시간 필터링을 통해 원천 배제합니다.  
...
* **AI 탐지 로직:**  
  ...
  * 회원들의 실루엣과 동선 벡터를 트래킹하여 특정 격자 구역에 인원이 밀집되는 패턴을 '히트맵(Heatmap)' 데이터로 실시간 시각화합니다.  
...
## **Ⅲ. 기술적 안전성: 비식별 정보 처리 및 로컬 엣지 연산**

개발 부서와 법무 팀이 가장 우려하는 **개인정보 보호법(CCTV 오용 및 초상권 침해)** 이슈를 원천 해결하기 위해 전 과정을 '완벽한 온디바이스(On-Device) 로컬 엣지 연산'으로 설계했습니다.

* CCTV 카메라 스트림(RTSP)은 외부 인터넷 망이 아닌, 센터 내부의 스위칭 허브를 통해 **젯슨 오린 보드로 로컬 전송**됩니다.  
* 인간 객체 감지 및 카운팅 연산 즉시 비디오 데이터는 전량 파기(No Storage)되며, 오직 비식별 가공된 **JSON 데이터 패킷**만 실시간 전송됩니다.  
...
### **🔲 SLIDE 3\. 시스템 구성도: 백엔드 AI 연산과 디스플레이 허브의 완벽한 절연**

* **비주얼 가이드:** 데이터의 흐름(Raw Stream ➡️ JSON ➡️ UI Render)이 막힘 없이 전달되는 고화질 흐름 블록도.  
* **스토리텔링 구사:** "개발실장님들이 가장 흔히 겪으시는 트러블슈팅은 연산 병목으로 인한 시스템 크래시(Crash)입니다. 저희는 이를 방지하기 위해 역할을 완전히 쪼개었습니다. AI 객체 탐지 연산만을 지옥처럼 전담하는 백엔드 엔진 '젯슨 오린'을 구동하고, 렌더링 및 디스플레이는 저렴하고 가벼운 전면의 안드로이드 기기가 전담합니다. 로컬 소켓 통신을 기반으로 하므로 유선망 수준의 속도로 렉 없는 실시간 갱신을 지원합니다\[cite: 2\]."

\[지점 CCTV 피드\] ──(RTSP)──\> \[Jetson Orin Super (백엔드)\]  
                                    │  
                         (로컬 JSON 가공 데이터 송출)  
                                    ▼  
...
### **🔲 SLIDE 4\. 유산소 존 알고리즘: ROI 기반 개별 트레드밀 점유 식별**

* **비주얼 가이드:** CCTV 화면 격자선(그린/레드 ROI 박스) 위에 사람의 실루엣이 겹쳤을 때, 활성화 상호작용이 표시되는 인공지능 탐지 목업 화면.  
* **스토리텔링 구사:** "전체 머리수를 세는 뻔한 비전 AI 기술이 아닙니다. 기존 설치된 CCTV 화면 속의 각 러닝머신 발판 및 싸이클 구동부를 개별 관심구역(ROI)으로 바코드 그리드 매핑합니다\[cite: 2\]. 인체 바운딩 박스가 해당 픽셀 영역과 ![][image1] 이상 중첩되며 모션 벡터가 발생할 때에만 '기구 사용 중'으로 트리거를 발생시킵니다. 카운터 직원이 직접 보지 않고도, 매장 러닝머신 전체 가용 상태가 1초 단위로 대시보드에 업데이트됩니다."
...
### **🔲 SLIDE 5\. 프리웨이트 존 알고리즘: 히트맵 기반 공간 혼잡 추정**
...
### **🔲 SLIDE 6\. 프로젝트 로드맵: 단독 가동 PoC에서 전사 앱 연동까지**

* **비주얼 가이드:** 간결하고 신속한 가로 마일스톤 흐름도.  
* **스토리텔링 구사:** "스포애니 본사와 처음부터 머리 아프게 데이터베이스(DB) 통합 및 서버 인프라 논의를 시작하지 않겠습니다. 그것은 개발 부서의 부담을 키워 계약 성사율만 떨어뜨립니다. 1단계로 스포애니 특정 가맹점 1곳의 기존 CCTV에 저희 젯슨 엣지 보드와 안드로이드 셋톱박스만 단독으로 가동하는 '단독형 엣지 패키지' 검토(PoC)를 진행합니다\[cite: 2\]. 현장에서 확실한 기술과 회원 만족도를 입증한 후, 2단계로 전 지점 도입 및 스포애니 모바일 앱 내 API 연동을 논의하여 도입 부담을 완전히 철산하겠습니다."
...
\[Phase 2 (PoC 통과 후 본 계약)\]  
 \* PoC 데이터 정량적 수치 기반 스포애니 전국 가맹점 순차 공급 확산\[cite: 3\].  
...
저희 에이아이큐브는 이미 실내 조도 간섭과 데이터 파편화 이슈가 완벽히 차단된 엣지 연산 인프라 및 전용 분산 처리 아키텍처 구성을 마쳤습니다\[cite: 2\]. 6월 미팅 시, 제안서 내 가이드라인으로 배치된 모든 더미 이미지들을 당사가 **룩앤필에 맞춰 직접 정밀 촬영한 고품질 실제 기기 팩킹 사진**으로 전량 수동 교체하여, 최종 발표용 슬라이드 덱으로 완벽하게 연출하여 피칭을 완성하겠습니다.
